...
void setup()
{
  ...
  digitalWrite(PI_CLOCK, LOW);
  pinMode(PI_CLOCK, INPUT);

  pinMode(PI_DATA, OUTPUT);
...
//Serial.println(N64_status.stick_y, DEC);
    ...
    // magic number 0x55
    for(i = 0; i < 4; i++) {
        digitalWrite(PI_DATA, LOW);
        delay(2);
        digitalWrite(PI_DATA, HIGH);
        delay(2);
    }
    ...
    for (i = 0; i < 16; i++) {
      if(N64_raw_dump[i] == 0) { digitalWrite(PI_DATA, LOW); delay(2); }
      else { digitalWrite(PI_DATA, HIGH); delay(2); }
    }
    for(i = 0; i < 8; i++) {
      if(N64_status.stick_x & (0x80 >> i) != 0) { digitalWrite(PI_DATA, HIGH); delay(2); }
      else { digitalWrite(PI_DATA, LOW); delay(2); }
    }
    for(i = 0; i < 8; i++) {
      if(N64_status.stick_y & (0x80 >> i) != 0)  { digitalWrite(PI_DATA, HIGH); delay(2); }
      else { digitalWrite(PI_DATA, LOW); delay(2); }
    }
