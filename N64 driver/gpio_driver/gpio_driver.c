...
#define TIMER_SEC    0
#define TIMER_NANO_SEC  1000 * 1000 /* 2 us */
...
/* IRQ number. */
static int irq_gpio21 = -1;

/* Blink timer vars. */
static struct hrtimer pi_clock_timer;
static ktime_t kt;
...
static char DataPinValue(void) {
		return GetGpioPinValue(GPIO_24);
}

static volatile unsigned int clock_i = 0;

static enum hrtimer_restart pi_clock_timer_callback(struct hrtimer *param) {

	switch (clock_i % 2) {
			case 0 : ClearGpioPin(GPIO_21); break;
			case 1 : SetGpioPin(GPIO_21); break;
	}
	
	clock_i++;
	
	//printk(KERN_INFO "gpio23: %d\ti mod 2 = %u\n", GetGpioPinValue(GPIO_23), clock_i % 2);
	hrtimer_forward(&pi_clock_timer, ktime_get(), kt);
	
	return HRTIMER_RESTART;
	
}

// for counting 32 bits to be sampled
static volatile int i = 0; 

static volatile char waiting_for_magic_number = 1, magic_number_buffer = 0x00;
static volatile int data_buffer = 0x00000000;

/* interrupt handler called when rising edge on GPIO_21 */
static irqreturn_t h_irq_gpio21(int irq, void *data)
{
	//int current_byte;
	//int shift_value;
	
	// wait for magic number sequence
	if(waiting_for_magic_number) {
			magic_number_buffer <<= 1;
			if(DataPinValue() == 0x00) {
				magic_number_buffer &= 0xfe;
			} else {
				magic_number_buffer |= 0x01;
			}
			if(magic_number_buffer == 0x55) {
					waiting_for_magic_number = 0;
			}
	} else {
		if(i == 32) {
			i = 0;
			waiting_for_magic_number = 1;
			gpio_driver_buffer[0] = (data_buffer >> 24) & 0x000000ff;
			gpio_driver_buffer[1] = (data_buffer >> 16) & 0x000000ff;
			gpio_driver_buffer[2] = (data_buffer >> 8) & 0x000000ff; 
			gpio_driver_buffer[3] = (data_buffer >> 0) & 0x000000ff;
			//printk(KERN_INFO "%x %x %x %x\n", gpio_driver_buffer[0], gpio_driver_buffer[1], gpio_driver_buffer[2], gpio_driver_buffer[3]);  
			data_buffer = 0;
		} else {
			data_buffer <<= 1;
			if(DataPinValue() == 0x00) {
				data_buffer &= 0xfffffffe;
			} else {
				data_buffer |= 0x00000001;
			}
...
int gpio_driver_init(void)
{
    ...
    hrtimer_init(&pi_clock_timer, CLOCK_MONOTONIC, HRTIMER_MODE_REL);
    kt = ktime_set(TIMER_SEC, TIMER_NANO_SEC);
    pi_clock_timer.function = &pi_clock_timer_callback;
    hrtimer_start(&pi_clock_timer, kt, HRTIMER_MODE_REL);
	
    /* Initialize gpio 21 ISR. */
      
		result = gpio_request_one(GPIO_21, GPIOF_DIR_OUT, "irq_gpio21");
		if(result != 0)
		{
			printk("Error: GPIO request failed!\n");
			goto fail_irq;
		}
		irq_gpio21 = gpio_to_irq(GPIO_21);
		result = request_irq(irq_gpio21,  h_irq_gpio21,
		IRQF_TRIGGER_RISING, "irq_gpio21", (void *)(h_irq_gpio21));
		if(result != 0)
		{
...
void gpio_driver_exit(void)
{
	...
	free_irq(irq_gpio21, h_irq_gpio21);
	gpio_free(GPIO_21);

    /* Release high resolution timer. */
    hrtimer_cancel(&pi_clock_timer);
